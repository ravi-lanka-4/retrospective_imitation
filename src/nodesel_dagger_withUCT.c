...
SCIP_DECL_NODESELINIT(nodeselInitDagger)
{
   ...
   for(i=0; i<nodeseldata->nfeasiblesols; i++){
      if(strcmp(nodeseldata->normfname, DEFAULT_FILENAME) != 0){
            FILE* cnormF = fopen(nodeseldata->normfname, "r");
            SCIP_CALL( SCIPfeatCreate(scip, &(nodeseldata->feasiblefeat[i]), cnormF, 
                                       SCIP_FEATNODESEL_SIZE + nodeseldata->probfeatsize + BUFF, 
                                       SCIP_FEATNODESEL_SIZE) );
            fclose(cnormF);
      }
...
{  /*lint --e{715}*/
   ...
   val = compareNodes(scip, nodeseldata, node1, node2);
   if (val != 0)
      return val;
   else
   {
   ...
   for(j=0; j<nodeseldata->nfeasiblesols; j++)
   {
      ...
      if (!(nodeseldata->solflag[j])){
         ...
         if (csolidx == -1){
            ...
            assert(SCIPnodeGetScore(node) != nodeseldata->feasiblescore[csolidx]);
