   ...
   )
{
   ...
   (*node)->index2      = -1;
   (*node)->index3      = -1;
   (*node)->boundtype   = SCIP_BOUNDTYPE_LOWER;
   (*node)->ub          = SCIPsetInfinity(set);
   (*node)->lb          = -SCIPsetInfinity(set);
...
SCIP_RETCODE SCIPnodeSetSolFlag(
  SCIP_NODE*     node,
  int            idx,
  OPT_FLAG       value
)
{
  assert(node != NULL);
  assert(node->solflag != NULL);
  assert(node->nsolflag > idx);
  node->solflag[idx] = value;
  return SCIP_OKAY;
}

SCIP_Bool SCIPnodeGetSolFlag(
  SCIP_NODE*     node,
  int            idx 
)
{
  ...
  assert(node->solflag != NULL);
  assert(node->nsolflag > idx);
  return node->solflag[idx];
}

SCIP_RETCODE SCIPnodeAllocSolFlag(
  SCIP_NODE* node,
  int size
)
{
  assert(node != NULL);
  if(node->solflag == NULL){
    int i=0;
    node->nsolflag = size;
    BMSallocMemoryArray(&(node->solflag), size);

    /* Initialize to false */
    for(i=0; i<size; i++){
      node->solflag[i] = FALSE;
    }
  }
  return SCIP_OKAY;
}
...
void SCIPnodeSetUbLb(
   SCIP_NODE*            node,               /**< node to update lower bound for */
   SCIP_Real             ub,                 /**< new estimated bound for the node */
   SCIP_Real             lb                  /**< new estimated bound for the node */
   )
{
   assert(node != NULL);
...
#undef SCIPnodeGetBranchBound
#undef SCIPnodeGetBoundType
#undef SCIPnodeGetSolIdx
#undef SCIPnodeSetSolIdx

/** returns whether node optimal */
OPT_FLAG  SCIPnodeIsOptimal(
   SCIP_NODE*            node                /**< node */
   )
{
  assert(node != NULL);
  int i=0;
  SCIP_Bool isoptimal = FALSE;
  SCIP_Bool set=FALSE;
  for(i=0; i < node->nsolflag; i++)
  {
    if ((node->solflag[i]) == MY_UNKNOWN)
      continue;
    isoptimal =  isoptimal || (node->solflag[i]); set=TRUE;
  }
  if (!set)
...
void SCIPnodeSetBranchBound(
   SCIP_NODE*            node,               /**< node to set optimal */
   SCIP_Real             bound 
   )
{
   assert(node != NULL);
   node->branchbound = bound;
}

void SCIPnodeSetIndex(
   SCIP_NODE*            node,               /**< node to set optimal */
   int                   val,
   SCIP_Bool             index
   )
